...
Las propuestas nuevas usan indicador 2 = `4` (fuente no especificada), sin URI ni código de autoridad inventados. Las subdivisiones se transfieren a sus subcampos, conservando el orden y las repeticiones. Si falta un subcampo del encabezamiento o una ocurrencia en el framework, la transferencia se detiene antes de modificar el formulario y avisa cuál agregar. Los metadatos $0 y $2 son opcionales: se omiten si no hay editor disponible, sin impedir la importación del término. Si no se puede guardar el código de fuente en $2, el indicador 2 pasa de `7` a `4`.
...
Los términos importados de Wikidata, UNESCO y LCSH también se incorporan al historial, aunque el framework no incluya $0 o $2. La selección queda pendiente hasta confirmar que el encabezamiento se guardó en el MARCXML de Koha. Si se retira o modifica antes de guardar, no se registra la selección original. Se guarda el tema y sus subdivisiones con el mismo conteo por registro que las propuestas locales; el historial no conserva la procedencia ni la URI de la autoridad externa.
